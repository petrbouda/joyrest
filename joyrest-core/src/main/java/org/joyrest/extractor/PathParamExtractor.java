...
public class PathParamExtractor implements BiFunction<RoutePart<?>, String, PathParam> {
     ...
     * (part which is gained from the client) and tries to create a path param.
     *
     * <p>If it is just string path, so this method will return @{code null}</>.<p/>
     * <p>If it is param path, so method will find out whether is possible to cast the
     * object or not and then can throw an validation exception<p/>
     *
     * @param routePart configured part
     * @param pathPart path from a client's call
     * @return path param derived from route and incoming call or @{code null} whether
     *          the path part is
